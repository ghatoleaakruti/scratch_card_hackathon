...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (user.mintedBadges?.gold) {
      return NextResponse.json({ success: false, message: "Gold badge already minted" }, { status: 400 })
    }

    // Check if user has enough tokens (100 for Gold)
    const requiredTokens = 100
    if (user.tokenBalance < requiredTokens) {
      return NextResponse.json(
        {
          success: false,
          message: `Insufficient token balance. You need ${requiredTokens} tokens to mint a Gold badge.`,
    ...
    const mintResult = await mintNFTBadge(user.walletAddress, "GOLD")
    ...
    const updatedUser = await updateUserTokenBalance(user.id, newBalance, "gold")

    return NextResponse.json({
      success: true,
      message: "Gold badge minted successfully!",
      transactionHash: mintResult.transactionHash,
      newBalance: updatedUser?.tokenBalance || newBalance,
    })
  } catch (error) {
    console.error("Gold badge minting error:", error)
    return NextResponse.json(
      {
        success: false,
        message: error instanceof Error ? error.message : "Failed to mint Gold badge",
      },
      { status: 500 },
    )
  }
